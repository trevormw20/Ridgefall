...
const JOB={
 ...
 ember:     {cid:'ember',name:'Ember Cantor',HP:27,ATK:3,MAG:10,DEF:2,RES:5,ACC:80,EVA:6,SPD:8,MOVE:4, skills:['Fire Spark','Blizzard','Ember Tile','Smoke Chant','Kindling'], react:'cinder', proj:'bolt'},
...
const SK={
 ...
 'Blizzard':{type:'magic',power:4,range:4,acc:85,aoe:'plus',status:'Hampered',proj:'ice',fx:{glyph:'❄',color:'#9fe6ff'},desc:'Ice magic. Bursts in a + (center and up/down/left/right), damaging every enemy caught and Slowing them (Hampered, MOVE -1).'},
...
function castAoe(att,cx,cy,sk){
  ...
  (function fr(now){const k=(now-t0)/total;att.aframe=Math.min(ANIM.attack.length-1,Math.floor(k*ANIM.attack.length));
    if(!fired&&k>=0.55){fired=true;
      ...
      projectiles.push({x:a.x,y:a.y-16,tx:b.x,ty:b.y-16,t:0,kind:sk.proj||att.proj||'bolt',onHit:()=>resolveAoe(att,tiles,cx,cy,sk)});}
    if(k<1)requestAnimationFrame(fr);else{att.anim='idle';att.aframe=0;}
  })(t0);
}
function resolveAoe(att,tiles,cx,cy,sk){
  shakeT=1;shakeMag=6;
  const fx=sk.fx||{glyph:'✸',color:'#ff9a4a'};
  pops.push({x:cx,y:cy,txt:fx.glyph,c:fx.color,t:0,lift:tt(cx,cy).high?HG_LIFT:0});
  let anyFoe=false;
...
function statusColor(s){return{Burn:'#ff7a3a',Slow:'#7ab0ff',Hampered:'#9fe6ff',Guarded:'#8fd0ff',Marked:'#ff5a5a',Exposed:'#ffb040',Rooted:'#b08050',Guided:'#e8c766',Riposte:'#d8d0a0',Intercept:'#8fd0ff'}[s]||'#fff';}
...
function drawProj(){for(const pr of projectiles){const x=pr.x+(pr.tx-pr.x)*pr.t,arc=Math.sin(pr.t*Math.PI)*18,y=pr.y+(pr.ty-pr.y)*pr.t-arc;
  ...
  else if(pr.kind==='ice'){ctx.fillStyle='#d6f4ff';ctx.beginPath();ctx.arc(x,y,4,0,Math.PI*2);ctx.fill();ctx.fillStyle='rgba(150,220,255,.4)';ctx.beginPath();ctx.arc(x,y,7,0,Math.PI*2);ctx.fill();}
